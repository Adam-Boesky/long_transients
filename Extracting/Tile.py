...
class Tile():
    ...
    def store_catalogs(self, out_parent_dir: str, overwrite: bool = False) -> str:
        ...
        out_subdir = f"{str(self.ztf_catalogs[self.bands[0]].image_metadata['fieldid']).zfill(6)}_{str(self.ztf_catalogs[self.bands[0]].image_metadata['ccdid']).zfill(2)}_{self.ztf_catalogs[self.bands[0]].image_metadata['qid']}"
        ...
        # Make subdirs, preserving any unrelated files that may already exist
        os.makedirs(outdir, exist_ok=True)
        os.makedirs(os.path.join(outdir, 'nan_masks'), exist_ok=True)
        os.makedirs(os.path.join(outdir, 'WCSs'), exist_ok=True)
        os.makedirs(os.path.join(outdir, 'EPSFs'), exist_ok=True)
