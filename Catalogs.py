...
class PSTARR_Catalog(Catalog):
    ...
    def get_data(self) -> Table:
        ...
        m.gKronMag, m.rKronMag, m.iKronMag, m.zKronMag, m.yKronMag, m.gKronMagErr, m.rKronMagErr, m.iKronMagErr, m.zKronMagErr, m.yKronMagErr,
        m.gPSFMag, m.rPSFMag, m.iPSFMag, m.zPSFMag, m.yPSFMag, m.gPSFMagErr, m.rPSFMagErr, m.iPSFMagErr, m.zPSFMagErr, m.yPSFMagErr
...
class ZTF_Catalog(Catalog):
    def __init__(self, ra: float, dec: float, data_dir: Optional[str] = None):
        super().__init__()
        ...
        self.ra_range, self.dec_range = self.sextractors['g'].get_coord_range()
...
def associate_tables_by_coordinates(table1: Table, table2: Table, max_sep: float = 1.0, prefix1: str = '', prefix2: str = '') -> Table:
    ...
    # Replace the ra and dec columns with the mean of the associated ras and decs
    associated_table['ra'] = (associated_table['ra'] + table2[idx[mask]]['ra']) / 2
    associated_table['dec'] = (associated_table['dec'] + table2[idx[mask]]['dec']) / 2
